    ...
    if (error instanceof HttpErrorResponse) {
      if (error.error instanceof ErrorEvent) {
        ...
        //   this.notificacion.error("No has iniciado sesión", 'Proceso Erroneo');
          this.messageService.add({ severity: 'success', summary: 'Proceso Erroneo', detail: 'No has iniciado sesión' });
          this.router.navigate(['login']);
        }
        if (error.status === 403) {
          console.log('redireccionar no autorizado');
        //   this.notificacion.error("No estás autorizado para estar aquí", 'Proceso Erroneo');
        this.messageService.add({ severity: 'success', summary: 'Proceso Erroneo', detail: 'No estás autorizado para estar aquí' });
          this.router.navigate(['login']);
        }
        if (error.status !== 200) {
          ...
          if(error.error.messages){
            ...
            this.messageService.add({ severity: 'success', summary: 'Proceso Erroneo', detail: ':(' });
            });
          }else{
            // this.notificacion.error(error.error.error, 'Proceso Erroneo');
            this.messageService.add({ severity: 'success', summary: 'Proceso Erroneo', detail: ':(' });
